...
function updateShareCodesCDN(url = 'http://adguard.b.freefrp.net/jd_cityShareCodes.json') {
  return new Promise(resolve => {
...
function readShareCode() {
  ...
  return new Promise(async resolve => {
    $.get({url: `http://share.turinglabs.net/api/v3/carnivalcity/query/0/`, 'timeout': 20000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
