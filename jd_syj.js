...
!(async () => {
  ...
  await getAuthorShareCode('http://adgurad.b.freefrp.net/jd_zz.json');
  await getAuthorShareCode('http://adgurad.b.freefrp.net/jd_zz.json');
  await getRandomCode();
...
async function getRandomCode() {
  await $.http.get({url: `https://code.c-hiang.cn/api/v1/jd/zuan/read/0`, timeout: 10000}).then(async (resp) => {
    if (resp.statusCode === 200) {
      try {
        let { body } = resp;
        body = JSON.parse(body);
